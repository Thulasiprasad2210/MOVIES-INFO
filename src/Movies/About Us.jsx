import React from 'react'

const AboutUs = () => {
  return (
    <>
     <div className='about'>
      <p>About Us</p>
      <h1>MOVIE INFO.</h1>
      <p>Our movie info website is a one-stop destination for film lovers to explore everything about their favorite movies. It provides detailed information including cast and crew, genres, ratings, release dates, and trailers in a clean and easy-to-use layout. From timeless classics to the latest releases, the platform is designed to help users discover, learn, and enjoy the world of cinema effortlessly. 🎬</p>
     </div> 
      <div className='team2'>
      <p>The Team</p>
      <h1>The People of Movie Info.</h1>
      </div>
      <div className='team1'>
     <div className='team'>
      <img src='https://www.thiings.co/_next/image?url=https%3A%2F%2Flftz25oez4aqbxpq.public.blob.vercel-storage.com%2Fimage-qT0qCttwF0fSi4qeWZj6vo2Za76keg.png&w=320&q=75' alt=''/>
      <h3>Catherine Moore</h3>
      <p>CEO</p>
      <p>moveinfo@site.com</p>
     </div>
     <div className='team'>
      <img src='https://www.thiings.co/_next/image?url=https%3A%2F%2Flftz25oez4aqbxpq.public.blob.vercel-storage.com%2Fimage-qT0qCttwF0fSi4qeWZj6vo2Za76keg.png&w=320&q=75' alt=''/>
      <h3>Curtis Webb</h3>
      <p>Executive Producer</p>
      <p>moveinfo@site.com</p>
     </div>
     <div className='team'>
      <img src='https://www.thiings.co/_next/image?url=https%3A%2F%2Flftz25oez4aqbxpq.public.blob.vercel-storage.com%2Fimage-qT0qCttwF0fSi4qeWZj6vo2Za76keg.png&w=320&q=75' alt=''/>
      <h3>Nick Ferguson</h3>
      <p>Production Manager</p>
      <p>moveinfo@site.com</p>
     </div>
     <div className='team'>
      <img src='https://www.thiings.co/_next/image?url=https%3A%2F%2Flftz25oez4aqbxpq.public.blob.vercel-storage.com%2Fimage-qT0qCttwF0fSi4qeWZj6vo2Za76keg.png&w=320&q=75' alt=''/>
      <h3>Andrew Simpson</h3>
      <p>Creative Development</p>
      <p>moveinfo@site.com</p>
     </div>
     <div className='team'>
      <img src='https://www.thiings.co/_next/image?url=https%3A%2F%2Flftz25oez4aqbxpq.public.blob.vercel-storage.com%2Fimage-qT0qCttwF0fSi4qeWZj6vo2Za76keg.png&w=320&q=75' alt=''/>
      <h3>Harlow Banks</h3>
      <p>Sales</p>
      <p>moveinfo@site.com</p>
     </div>
     <div className='team'>
      <img src='https://www.thiings.co/_next/image?url=https%3A%2F%2Flftz25oez4aqbxpq.public.blob.vercel-storage.com%2Fimage-qT0qCttwF0fSi4qeWZj6vo2Za76keg.png&w=320&q=75' alt=''/>
      <h3>Joe Priestley</h3>
      <p>Technical Director</p>
      <p>moveinfo@site.com</p>
     </div>
     <div className='team'>
      <img src='https://www.thiings.co/_next/image?url=https%3A%2F%2Flftz25oez4aqbxpq.public.blob.vercel-storage.com%2Fimage-qT0qCttwF0fSi4qeWZj6vo2Za76keg.png&w=320&q=75' alt=''/>
      <h3>Kevin Jen</h3>
      <p>Financial Director</p>
      <p>moveinfo@site.com</p>
     </div>
     <div className='team'>
      <img src='https://www.thiings.co/_next/image?url=https%3A%2F%2Flftz25oez4aqbxpq.public.blob.vercel-storage.com%2Fimage-qT0qCttwF0fSi4qeWZj6vo2Za76keg.png&w=320&q=75' alt=''/>
      <h3>Sharon Page</h3>
      <p>Office Manager</p>
      <p>moveinfo@site.com</p>
     </div>
     </div>

       <div className='end'>
        <div className='footer'>
            <p>Movies Info.</p>
            <h1>See it First</h1>
            <label for="" className='label'>Enter your email here*</label> <br/>
            <input className='input' type="email" placeholder='Type Here....'/> <br/>
            <label className='check'><input type="checkbox" />Yes, Subscribe me to Newsletter. *</label>
            <button className='btn'>SUBSCRIBE</button>
        </div>
      </div>
        <hr/>
        <div>
          <footer className='copy'>
            <p>&copy; Reserved by MOVIES INFO. 2025</p>
            <button>
            <a href="https://www.instagram.com/accounts/login/?hl=en" target='blank'><img src='https://cdn-icons-png.flaticon.com/128/1384/1384015.png' alt=''/></a>
            </button>
            <button>
            <a href="https://x.com/" target='blank'><img src='https://cdn-icons-png.flaticon.com/128/739/739257.png' alt=''/></a>  
            </button>
            <button>
            <a href="https://www.google.com/?zx=1766555920036&no_sw_cr=1" target='blank'><img src='https://cdn-icons-png.flaticon.com/128/6244/6244710.png' alt=''/> </a>
            </button>
            <button>
            <a href="https://www.facebook.com/login/" target='blank'><img src='https://cdn-icons-png.flaticon.com/128/2168/2168281.png' alt=''/> </a>
            </button>
          </footer>
          </div>

    </>
  )
}

export default AboutUs
